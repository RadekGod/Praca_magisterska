...
def _preprocess_image(
    ...
    """Preprocessing jak w treningu fuzji:

    - wejście: (H,W,4) uint8 [0..255] = [R,G,B,SAR]
    - RGB: /255 + normalizacja ImageNet mean/std
    - SAR: /255 + (opcjonalnie) normalizacja global/per-sample

    Zwraca tensor (1,4,H,W) float32.
    """
    if img.ndim != 3 or img.shape[-1] != 4:
        raise ValueError(f"Fusion expects image (H,W,4), got {img.shape}")

    x = img.astype(np.float32) / 255.0  # H,W,4

    # RGB (pierwsze 3 kanały)
    ...
    if sar_normalize == "global" and sar_mean is not None and sar_std is not None:
        ...
        x[..., sar_ch] = (x[..., sar_ch] - float(sar_mean)) / std
...
def _load_fusion_image(image_path: str) -> tuple[np.ndarray, str]:
    """Loader dopasowany do folderu `results/obrazy/fusion/oryginalne`.

    W tym folderze obrazy są w parach:
      - TrainArea_XXXX_RGB.tif  (3 kanały)
      - TrainArea_XXXX_SAR.tif  (1 kanał)

    Skrypt ma wspierać tylko ten format.

    Zwraca: (img_4ch [R,G,B,SAR], georef_source_path)
    gdzie georef_source_path to plik RGB (dla CRS/transform).
    """
    if not image_path.lower().endswith("_rgb.tif"):
        raise ValueError(
            "Obsługiwany jest tylko format *_RGB.tif z folderu results/obrazy/fusion/oryginalne. "
            f"Dostałem: {image_path}"
        )

    rgb_path = image_path
    sar_path = image_path[:-8] + "_SAR.tif"  # zamiana sufiksu _RGB.tif -> _SAR.tif
    ...
    if rgb.shape[0] != sar.shape[0] or rgb.shape[1] != sar.shape[1]:
        raise ValueError(f"RGB/SAR shape mismatch: rgb={rgb.shape} sar={sar.shape} for {rgb_path}")

    return np.dstack([rgb, sar[..., None]]), rgb_path
...
def run_inference(
    ...
    """Inferencja dla pojedynczego obrazu fusion (4 kanały) i zapis kolorowej maski."""
    img, georef_path = _load_fusion_image(image_path)

    x = _preprocess_image(img, sar_normalize=sar_normalize, sar_mean=sar_mean, sar_std=sar_std).to(device)
    ...
    rgb_mask = logits_to_mask_rgb(logits)

    # Zapis GeoTIFF z georeferencją (o ile jest)
    try:
...
def main():
    ...
    parser.add_argument(
        ...
        help="Ścieżka wyjściowa (plik albo folder, analogicznie jak --image_path)",
    )
    ...
    # W folderze są pary *_RGB.tif + *_SAR.tif. Przetwarzamy TYLKO *_RGB.tif.
    ...
    # dodatkowa walidacja: czy dla każdego RGB jest SAR
    missing_sar = []
    for fn in rgb_files:
